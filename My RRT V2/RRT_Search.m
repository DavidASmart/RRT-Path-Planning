...
% 3/3/18
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Path, Cost, Time] = RRT_Search (StartNode, GoalNode, res, limit, nsmooth, params, dispRRT)
% params: 
...
%     InitDisplay(StartNode, GoalNode, res)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Expand Nodes
    [GoalReached] = TryStraightline(GoalNode, res); % initial straightline attempt
    k = 1; i = 3; % init variables ialize
    while ~GoalReached % until the goal is reached

        % try a random point
        [GoalReached, h] = RRT_Move(StartNode, GoalNode, res, i, limit, h, k, params, dispRRT); 
        k = k + 1; % count of samples, required for Halton Sequence
        i = i + 0.1; % expanding percentage
        
        if params(4) == 1
            % then try a straight line to the goal
            [GoalReached, h] = RRT_TryStraightline(GoalNode, res, i, limit, h, params, dispRRT);
        end
